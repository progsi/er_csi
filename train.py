...
for epoch in range(2):  # adjust as needed
    for batch in train_dataloader:
        ...
        positive_example["yt_title"][0]
        negative_examples = [s["yt_title"][0] for s in negative_examples]
        
        # Forward pass
        ref_embedding = model(ref_sentence)
        positive_embedding = model(positive_example)
        negative_embeddings = [model(neg_example) for neg_example in negative_examples]
